...
> *QA 엔지니어의 꼼꼼한 시선으로, 결함 없는 제품을 만드는 개발자를 목표로 합니다.*
...
QA Engineer → Developer 전환 과정을 담은 개인 포트폴리오 겸 실사용 대시보드 SPA입니다.
...
이직 준비 기간의 할 일과 일정을 기록하려고 만들기 시작했는데, 어차피 포트폴리오 페이지도
필요하다는 걸 깨닫고 하나로 합쳤습니다. 그래서 "보여주기 위한 포트폴리오"가 아니라 지금도
매일 실제로 쓰는 도구에서 출발한 프로젝트입니다.
...
```mermaid
flowchart TD
    User(["사용자"]) -->|"hashchange"| Router["Hash 기반 라우터<br/>(location.hash)"]
    Router --> Home["Home"]
    Router --> Portfolio["Portfolio"]
    Router --> Dashboard["Dashboard<br/>(Kanban/Calendar/Archive)"]
    Router --> Resume["resume.html<br/>(별도 페이지)"]

    Dashboard -->|"localStorage.setItem 감지"| Debounce["2초 디바운스"]
    Debounce -->|"upload"| Firestore[("Firebase Firestore")]
    Firestore -->|"타임스탬프 비교 후 다운로드"| Dashboard

    Portfolio -->|"GitHub API"| GH["api.github.com<br/>PushEvent 집계"]
```

- **라우팅**: 외부 프레임워크 없이 `window.location.hash`와 `hashchange` 이벤트만으로 SPA 페이지 전환을 구현했습니다. 새로고침 시에도 마지막 위치가 복원됩니다.
- **동기화**: `localStorage.setItem`을 오버라이드해, 할 일/메모/타임라인 관련 키가 변경될 때만 2초 디바운스 후 Firestore에 업로드합니다. 페이지 로드 시에는 서버와 로컬의 타임스탬프를 비교해 서버가 더 최신이면 자동으로 다운로드하는데, 이때는 `originalSetItem`을 직접 호출해 오버라이드 로직이 재귀적으로 발동하지 않도록 방지합니다.
- **GitHub 연동**: 포트폴리오 페이지의 커밋 잔디밭은 `api.github.com`에서 실제 PushEvent를 가져와 날짜별로 집계해 렌더링합니다.
